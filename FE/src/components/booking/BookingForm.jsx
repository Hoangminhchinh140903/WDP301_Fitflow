...
const FALLBACK_CATEGORY = 'Đặt lịch thử vợt/sân';
...
export default function BookingForm({ onCancel, onSuccess, selectedProduct = null }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || err.message || 'Không thể đặt lịch thử vợt/sân lúc này. Vui lòng thử lại.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <section className="rounded-2xl border border-amber-100 bg-amber-50/70 p-3">
        ...
        {selectedProduct?.name ? (
          ...
          <div className="rounded-xl bg-white p-2.5 text-sm text-slate-600">Chưa chọn mẫu cụ thể, bạn có thể tiếp tục đặt lịch thử vợt/sân.</div>
        )}
      </section>
