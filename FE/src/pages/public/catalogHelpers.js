...
export const toCategoryNode = (item = {}) => ({
  ...
  children: Array.isArray(item.children)
    ? item.children
        .map((child) => toCategoryNode(child))
...
export const buildSidebarTree = (rawCategories = []) => {
  ...
  const hasChildrenFromApi = categories.some(
  ...
  const parentIndex = topLevel.findIndex((category) =>
    isAoDaiParent(category.displayName),
  );
  const parent = parentIndex >= 0 ? topLevel[parentIndex] : null;
  if (parentIndex >= 0) topLevel.splice(parentIndex, 1);

  const mergedChildren = mergeUniqueChildren([
    ...(parent?.children || []),
    ...children,
  ...
  const groupedParent = {
    ...
    displayName: parent?.displayName || "Đồ Thể Thao Cho Thuê",
    count:
      parent?.count ||
      mergedChildren.reduce((sum, item) => sum + item.count, 0),
    slug: parent?.slug || "",
...
export const mapRentDisplayName = (name = "") => {
  ...
  if (
    ["phu-kien", "phu-kien-di-kem", "phu-kien-chup-anh-cho-thue"].includes(
      normalized,
    ) ||
    normalized.startsWith("phu-kien-")
  ) {
    return "Phụ kiện thể thao chụp ảnh cho thuê";
  }
  return name;
};
...
const expandRentCandidates = (candidates = []) => {
  const set = new Set(
    candidates.map((item) => normalizeToken(item)).filter(Boolean),
  );
  ...
  for (const [canonical, aliases] of Object.entries(RENT_CATEGORY_ALIASES)) {
    const matchedAlias = aliases.some(
      (alias) =>
        ...
        values.some((item) => item.startsWith(`${alias}-`)),
    );
    if (matchedAlias) {
      set.add(canonical);
...
export const resolveCategoryValueFromKeyword = ({
  keyword = "",
  nodes = [],
  mode = "shop",
}) => {
  ...
  const normalizedKeyword =
    mode === "rent" ? normalizeRentKeyword(keyword) : normalizeText(keyword);
  const matched = nodes.find((item) =>
    (mode === "rent"
      ? expandRentCandidates([item.value, item.displayName, item.slug])
      : [item.value, item.displayName, item.slug].map((candidate) =>
...
export const collectVariantOptions = (products = []) => {
  ...
  products.forEach((product) => {
    ...
    const fallbackSizeOptions = Array.isArray(product?.sizeOptions)
      ? product.sizeOptions
      : [];
...
export const hasDateOverlap = (
  ...
  if ([start, end, bStart, bEnd].some((value) => Number.isNaN(value)))
    return false;
