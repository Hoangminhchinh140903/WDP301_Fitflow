...
const sendResetPasswordEmail = async ({ to, name, resetLink, expiresInMinutes }) => {
  ...
  const subject = 'Đặt lại mật khẩu tài khoản FITFLOW';
  const text = [
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.5;color:#111827">
      <h2>Đặt lại mật khẩu tài khoản FITFLOW</h2>
...
const sendGuestVerificationEmail = async ({ to, code, expiresInMinutes }) => {
  const transporter = createTransporter();
  const from = process.env.SMTP_FROM || `FITFLOW <${process.env.SMTP_USER}>`;
  const subject = 'Mã xác minh thanh toán FITFLOW';
  const text = [
    'Xin chào,',
    '',
...
const sendStaffInvitationEmail = async ({ to, name, inviterName, acceptLink, expiresInHours }) => {
  ...
  const subject = 'Thu moi tham gia tai khoan nhan su FITFLOW';
  const text = [
    `Xin chao ${displayName},`,
    '',
    `${displayInviterName} vua moi ban tham gia he thong nhan su FITFLOW.`,
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.5;color:#111827">
      <h2>Thu moi tham gia nhan su FITFLOW</h2>
      <p>Xin chao ${displayName},</p>
      <p>${displayInviterName} vua moi ban tham gia he thong nhan su FITFLOW.</p>
...
const sendInvoiceEmail = async ({ to, buyerName, orderCode, totalAmount, invoiceNo, invoiceDate, pdfBuffer, pdfFilename }) => {
  ...
  const subject = `✅ Xác nhận đơn hàng ${orderCode} – FITFLOW`;
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#f8fbff;padding:0;border-radius:16px;overflow:hidden">
      <div style="background:#1975d2;padding:32px 40px;text-align:center">
        <h1 style="color:#ffffff;margin:0;font-size:24px;font-weight:700">FITFLOW</h1>
      ...
      <div style="background:#ffffff;padding:32px 40px">
        ...
        <p style="color:#6b7280;margin:0 0 24px;font-size:14px;line-height:1.6">
          ...
          Cảm ơn bạn đã đặt hàng tại FITFLOW. Đơn hàng của bạn đã được ghi nhận và đang được xử lý.
        </p>
      ...
      <div style="background:#f0f7ff;padding:20px 40px;text-align:center">
        <p style="color:#9ca3af;font-size:12px;margin:0">
          Email này được gửi tự động bởi hệ thống FITFLOW. Vui lòng không trả lời email này.
        </p>
      </div>
    </div>
  `;
