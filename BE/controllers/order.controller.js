...
const ensureSaleStockAvailable = async (normalizedItems = []) => {
  ...
  const expandedConditionLevels = requestedConditionLevels.flatMap(level => level === 'New' ? ['New', 'Good'] : level === 'Used' ? ['Used', 'Damaged'] : [level]);

  const candidateInstances = await ProductInstance.find({
    productId: { $in: requestedProductIds },
    lifecycleStatus: 'Available',
    conditionLevel: { $in: expandedConditionLevels },
  ...
  const isEquivalentCondition = (dbLevel, reqLevel) => {
    if (reqLevel === 'New') return dbLevel === 'New' || dbLevel === 'Good';
    if (reqLevel === 'Used') return dbLevel === 'Used' || dbLevel === 'Damaged';
    return dbLevel === reqLevel;
  };

  for (const { productId, conditionLevel, size, count } of grouped) {
    const available = unblockedCandidates.filter((i) => (
      String(i.productId) === String(productId)
      && isEquivalentCondition(i.conditionLevel, conditionLevel)
      && matchesSize(i, size)
    )).length;
...
const assignSaleInstances = async (normalizedItems = [], saleOrderId = null, session = null) => {
  ...
  for (const item of normalizedItems) {
    ...
    const requestedInstanceId = mongoose.isValidObjectId(item?.productInstanceId)
      ...
      : null;
    for (let i = 0; i < qty; i++) {
      const sizeQuery = size === 'FREE SIZE'
        ? { $in: ['FREE SIZE', 'free size', '', null] }
        : { $regex: new RegExp(`^${size}$`, 'i') };
      
      const conditionQuery = conditionLevel === 'New' ? { $in: ['New', 'Good'] } : { $in: ['Used', 'Damaged'] };

      const query = {
        productId: item.productId,
        lifecycleStatus: 'Available',
        conditionLevel: conditionQuery,
        size: sizeQuery,
  ...
  try {
    ...
    if (role === 'owner' || role === 'staff') {
      return res.status(403).json({
        success: false,
        message: 'Tài khoản owner/staff không thể mua hàng ở trang khách. Vui lòng dùng tài khoản khách hàng.',
      });
    }
