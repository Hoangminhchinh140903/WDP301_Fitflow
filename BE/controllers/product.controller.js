...
const getProducts = async (req, res) => {
  try {
    ...
    // Mua: ưu tiên còn hàng (Available) lên đầu, sau đó tồn cao hơn, cuối cùng mới nhất — ổn định phân trang.
    if (purpose === 'buy') {
      filteredProducts.sort((a, b) => {
        const aq = Math.max(0, Number(a?.availableQuantity ?? 0));
        const bq = Math.max(0, Number(b?.availableQuantity ?? 0));
        const aOk = aq > 0;
        const bOk = bq > 0;
...
const listOwnerProducts = async (req, res) => {
  try {
    ...
    // Ưu tiên còn hàng (Available) → tổng có sẵn cao hơn → tổng instance → mới tạo hơn (giữ hành vi gần sort cũ).
    data.sort((a, b) => {
      const aq = Math.max(0, Number(a.availableQuantity ?? 0));
      const bq = Math.max(0, Number(b.availableQuantity ?? 0));
      const aOk = aq > 0;
      ...
      if (aOk !== bOk) return aOk ? -1 : 1;
      if (bq !== aq) return bq - aq;
      const at = Math.max(0, Number(a.totalQuantity ?? 0));
      const bt = Math.max(0, Number(b.totalQuantity ?? 0));
...
const updateOwnerProduct = async (req, res) => {
  try {
    ...
    const uploadedImages = await uploadOwnerImages(req.files);
    const nextPayloadImages = uploadedImages.length > 0
      ? normalizeImages([...(payload.images || []), ...uploadedImages])
      : (Array.isArray(payload.images) ? payload.images : existing.images);

    const nextPayload = {
      ...payload,
      images: nextPayloadImages,
      variants: [],
      imageUrl: Array.isArray(nextPayloadImages) && nextPayloadImages.length > 0 ? nextPayloadImages[0] : '',
    };
